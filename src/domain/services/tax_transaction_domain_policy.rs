//! Domain policy for TaxTransaction aggregate
//!
...
/// Domain policy for TaxTransaction — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<TaxTransaction>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type TaxTransactionDomainPolicy = PermitAllPolicy<TaxTransaction>;
