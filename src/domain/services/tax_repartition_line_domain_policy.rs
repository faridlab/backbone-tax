//! Domain policy for TaxRepartitionLine aggregate
//!
...
/// Domain policy for TaxRepartitionLine — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<TaxRepartitionLine>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type TaxRepartitionLineDomainPolicy = PermitAllPolicy<TaxRepartitionLine>;
