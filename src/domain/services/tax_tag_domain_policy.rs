//! Domain policy for TaxTag aggregate
//!
...
/// Domain policy for TaxTag — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<TaxTag>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type TaxTagDomainPolicy = PermitAllPolicy<TaxTag>;
