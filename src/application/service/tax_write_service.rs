...
//! service's tenancy decorator installs org_unit_id + the row-level fences. Every transaction
//! relays the AMBIENT request org scope (`backbone_orm::org_scope::bind_org_scope_on`) when the
//! composing service bound one: the decorator-installed fences govern which rows a verb can see
//! and write, and a decorated host's per-unit uniques back the duplicate-code checks. A
//! deployment that runs unfenced gets an unfenced module. Two installer probes keep the
//! documented LEGACY TWIN: `template_id_by_code` / `tag_id_by_code` take a company id because
//! chart + tax-template installers run OUTSIDE a request scope (bootstrap paths) and key their
//! idempotency on one company — the probes construct the single-company scope from that id and
//! read under it. The module never guesses a company of its own.
//!
//! All SQL lives in the repositories (tax_category / tax_template / tax_template_row /
//! withholding_category / company_tax_settings / tax_tag / tax_repartition_line); this service
//! only orchestrates. 4-layer rule.
...
pub struct NewTemplate {
    ...
    /// owning unit's settings `default_exigibility` (itself defaulting to `on_invoice`).
    pub tax_exigibility: Option<String>,
    /// Transition account for `on_payment` templates. `None` ⇒ the owning
    /// unit's settings transition account.
    pub cash_basis_transition_account_id: Option<Uuid>,
}

/// PUT body for the owning unit's tax posture. Full-replace semantics.
#[derive(Debug, Clone)]
...
impl TaxWriteService {
    ...
    pub async fn create_category(&self, c: NewCategory) -> Result<Uuid, TaxError> {
        ...
        if repos.find_by_code_on(&mut tx, &c.code).await?.is_some() {
            return Err(TaxError::DuplicateCode(c.code));
        }
        let r = TaxCategoryRepository::insert_on(
            &mut *tx,
            &NewTaxCategoryRow {
                id,
                code: &c.code,
    ...
    /// the schema).
    async fn verify_caba_transition(
        ...
        let schema_present: Option<bool> =
    ...
    /// Scoped lookup of an active template by its per-unit-unique code — the LEGACY TWIN
    /// probe (ADR-0029). The composition installer (chart + companion tax templates) runs
    /// outside a request scope and keys its idempotency on one company, so the probe
    /// constructs the documented single-company scope from that id and reads under it:
    /// a decorated deployment's fence admits exactly that unit's rows; a bare one reads
    /// by code alone. `None` = no such template for the named company.
    ...
    /// Scoped lookup of an active tag by its per-unit-unique code — the tag twin
    ...
    /// of a duplicate-code error. `None` = no such tag for the named company.
    ...
    pub async fn create_template(&self, t: NewTemplate) -> Result<Uuid, TaxError> {
        ...
        if exigibility == "on_payment" {
            ...
            Self::verify_caba_transition(&mut tx, account).await?;
        } else {
        ...
        let id = Uuid::new_v4();
        let insert = TaxTemplateRepository::insert_on(
            &mut *tx,
            &NewTaxTemplateRow {
                id,
                code: &t.code,
        ...
        if let Err(e) = insert {
            return match e {
                ref e if Self::is_dup(e) => Err(TaxError::DuplicateCode(t.code)),
                e => Err(e.into()),
            };
        }
    ...
    /// The owning unit's tax posture, if configured. `None` ⇒ documented defaults
    ...
    /// PUT semantics: insert-or-update the owning unit's single live settings row.
    ...
    pub async fn add_repartition_line(&self, r: NewRepartitionLine) -> Result<Uuid, TaxError> {
        ...
        let id = Uuid::new_v4();
        let res = TaxRepartitionLineRepository::insert_on(
            &mut tx,
            &NewTaxRepartitionLineRecord {
                id,
    ...
    pub async fn add_row(&self, row: NewTemplateRow) -> Result<Uuid, TaxError> {
        ...
        TaxTemplateRowRepository::insert_on(
            &mut *tx,
    ...
    pub async fn create_withholding(&self, w: NewWithholding) -> Result<Uuid, TaxError> {
        ...
        // Reject an overlapping window for the same code within this unit (council 2026-07-03)
        ...
        let id = Uuid::new_v4();
        WithholdingCategoryRepository::insert_on(
            &mut *tx,
