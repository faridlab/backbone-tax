...
-- Create tax_rounding_method enum type. The existence check is namespace-scoped:
-- pg_type is not, so a bare typname check lets a same-named enum owned by a sibling
-- module in a shared database (e.g. billing.tax_exigibility) suppress this type's
-- creation and leave every later reference to it unresolvable.
DO $$
BEGIN
    IF NOT EXISTS (
...
-- Create tax_exigibility enum type (same namespace-scoped guard).
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_type t JOIN pg_namespace n ON n.oid = t.typnamespace
